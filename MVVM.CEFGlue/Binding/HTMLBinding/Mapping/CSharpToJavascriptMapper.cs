...
namespace MVVM.CEFGlue.HTMLBinding
{
    internal class CSharpToJavascriptMapper 
    {
        ...
        public CSharpToJavascriptMapper(IWebView context, IJSCBridgeCache icacher)
        {
            _IWebView = context;
        ...
        private IJSCSGlue InternalMap(object ifrom, object iadditional=null)
        {
            ...
            if (ifrom is ICommand)
                return new JSCommand(_IWebView, ifrom as ICommand);
        ...
        {
            res = new JSArray(this._IWebView, source.Cast<object>().Select(s => Map(s)), source);
